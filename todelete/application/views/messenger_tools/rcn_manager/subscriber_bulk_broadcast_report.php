<section class="section section_custom">
  <div class="section-header">
    ...
    <div class="section-header-button">
      <a href="<?php echo base_url('messenger_bot_broadcast/rcn_create_subscriber_broadcast_campaign'); ?>" class="btn btn-primary"><i class="fas fa-plus-circle"></i> <?php echo $this->lang->line("Create Campaign"); ?></a>
    </div>
...
<script>
	...
	table1 = $("#mytable").DataTable({
	  ...
	  pageLength: 10,
	  ajax: {
	      url: base_url+'messenger_bot_broadcast/rcn_subscriber_broadcast_campaign_data',
	      type: 'POST',
	      data: function ( d )
	      {
	...
	$("document").ready(function(){	   
	    ...
	    $(document).on('click','.sent_report',function(e){
    	  ...
    	  setTimeout(function(){
	    	$.ajax({
	    		type:'POST' ,
	    		url:"<?php echo site_url();?>messenger_bot_broadcast/rcn_campaign_sent_status",
	    		...
	    		success:function(response){
	    			...
	    			if (table2 == '')
	    			{
	    			  ...
	    			  table2 = $("#mytable2").DataTable({
	    			      ...
	    			      pageLength: 10,
	    			      ajax: {
	    			          url: '<?php echo base_url("messenger_bot_broadcast/rcn_campaign_sent_status_data"); ?>',
	    			          type: 'POST',
		...
		.then((willDelete) => {
			if (willDelete) 
			{
			    ...
			    $.ajax({
			       context: this,
			       type:'POST' ,
			       url: "<?php echo base_url('messenger_bot_broadcast/rcn_restart_campaign')?>",
		...
		.then((willDelete) => {
			if (willDelete) 
			{
			    ...
			    $.ajax({
			       context: this,
			       type:'POST' ,
			       url: "<?php echo base_url('messenger_bot_broadcast/rcn_force_reprocess_campaign')?>",
			       data: {id:id,csrf_token:csrf_token},
			       success:function(response)
			       {
				       	$(this).parent().prev().removeClass('btn-progress btn-primary').addClass('btn-outline-primary');
		...
		.then((willDelete) => {
			if (willDelete) 
			{
			    ...
			    $.ajax({
			       context: this,
			       type:'POST' ,
			       url: "<?php echo base_url('messenger_bot_broadcast/rcn_ajax_campaign_pause')?>",
			       data: {table_id:table_id,csrf_token:csrf_token},
			       success:function(response)
			       {
				       	$(this).parent().prev().removeClass('btn-progress btn-primary').addClass('btn-outline-primary');
		...
		.then((willDelete) => {
			if (willDelete) 
			{
			    ...
			    $.ajax({
			       context: this,
			       type:'POST' ,
			       url: "<?php echo base_url('messenger_bot_broadcast/rcn_ajax_campaign_play')?>",
			       data: {table_id:table_id,csrf_token:csrf_token},
			       success:function(response)
			       {
				       	$(this).parent().prev().removeClass('btn-progress btn-primary').addClass('btn-outline-primary');
		...
		.then((willDelete) => {
			if (willDelete) 
			{
			    ...
			    $.ajax({
			       context: this,
			       type:'POST' ,
			       url: "<?php echo base_url('messenger_bot_broadcast/rcn_subscriber_delete_campaign')?>",
			       data: {id:id,csrf_token:csrf_token},
			       success:function(response)
			       {
				       	$(this).parent().prev().removeClass('btn-progress btn-primary').addClass('btn-outline-primary');
