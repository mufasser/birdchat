...
class Admin extends Home
{
    ...
    {       
        ...
        if ($_POST) 
        {
            ...
            $this->form_validation->set_rules('pusher_app_id',   '<b>'.$this->lang->line("Pusher App Id").'</b>',                 'trim');
            $this->form_validation->set_rules('pusher_app_key',   '<b>'.$this->lang->line("Pusher App Key").'</b>',                 'trim');
            $this->form_validation->set_rules('pusher_cluster',   '<b>'.$this->lang->line("Pusher Cluster").'</b>',                 'trim');
            $this->form_validation->set_rules('pusher_app_secret',   '<b>'.$this->lang->line("Pusher App Secret").'</b>',                 'trim');
            ...
            else 
            {
                ...
                $pusher_app_key=addslashes(strip_tags($this->input->post('pusher_app_key', true)));
                $pusher_cluster=addslashes(strip_tags($this->input->post('pusher_cluster', true)));
                $pusher_app_id=addslashes(strip_tags($this->input->post('pusher_app_id', true)));
                $pusher_app_secret=addslashes(strip_tags($this->input->post('pusher_app_secret', true)));
                ...
                $app_pusher_config_data = "<?php ";
                $app_pusher_config_data.= "\n\$config['pusher_app_id'] = '".$pusher_app_id."';\n";
                $app_pusher_config_data.= "\n\$config['pusher_app_key'] = '".$pusher_app_key."';\n";
    ...
    {           
        ...
        $display_columns = array("#","CHECKBOX",'user_id','avatar','name', 'email','fb_id','package_name', 'status', 'user_type','expired_date', 'actions', 'add_date','last_login_at','last_login_ip');
        ...
        foreach ($info as $key => $value) 
        {
            ...
            if($this->is_demo=='1')  $info[$i]["fb_id"] ="XXXXXXXXX";
...
<li>Requested Amount : #REQUESTED_AMOUNT#</li>
        ...
        if($this->addon_exist("email_optin_form_builder")) {

            $temp6 = array(
                'subject' => "#APP_NAME# | Email Activation",
                'message' => '<b><h2>Confrim your Subscription</h2></b>
<p>You received this email because you opted in for our email updates. Click the below confirmation link to confirm the subscription.</p>
<a href="#OPTIN_URL#">Confirm Your Email<a><br>
Thank you,<br>
<a href="#APP_URL#">#APP_NAME#</a> Team'
            );
            array_push($data['default_values'], $temp6);
