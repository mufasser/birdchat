...
class Payment extends Home
{
    ...
    public function accounts_action()
    {
        ...
        if ($_POST) 
        {
            ...
            $this->form_validation->set_rules('marcado_mode','<b>'.$this->lang->line("Mercadopago Supported Country").'</b>','trim');
            ...
            else 
            {
                ...
                $mercadopago_mode=strip_tags($this->input->post('mercadopago_mode',true));
        ...
        if ($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type') == 'Member') 
        {
            ...
            if(!empty($payment_config)) 
            {
                ...
                $mercadopago_access_token = isset($payment_config[0]['mercadopago_access_token']) ? $payment_config[0]['mercadopago_access_token'] : '';
                 ...
                 $mercadopago_mode = isset($payment_config[0]['mercadopago_mode']) ? $payment_config[0]['mercadopago_mode'] : 'live';
            ...
            if($mercadopago_public_key!='' && $mercadopago_access_token!='')
            {
                ...
                $this->mercadopago->mercadopago_mode=$mercadopago_mode;
