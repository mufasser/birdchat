...
{ 
  ...
  public function setting()
    {
        $setting = APPPATH . 'views/setting/setting.php';
        if (file_exists($setting)) {
            $this->load->view('setting/setting');
        } else {
            echo "File does not exist: " . $setting;
        }
    }
    public function inbox()
    {
        ...
        // Fetch data from the whatsapp_messages table
        // $query = $this->db->get('whatsapp_messages');
        $query = $this->db->select('message_from')
        ...
        ->from('whatsapp_messages')
        ->where('message_from', $result['message_from'])
        ->get()->result();

        $inbox_file_path = APPPATH . 'views/inbox/inbox.php';
        if ($result) {
            // Load the broadcast view and pass the result rows as data
            $this->load->view('inbox/inbox', ['messages' => $result]);
        } else {
            $this->load->view('inbox/inbox');
        }
    }
  public function broadcast()  {   
 $broadcast_file_path = APPPATH . 'views/broadcast/broadcast.php';  
      if (file_exists($broadcast_file_path)) {          
              $this->load->view('broadcast/broadcast');     
   } else {            echo "File does not exist: " . $broadcast_file_path;        } 
 }  public function birdchat_ai()  {    $birdchat_ai_file_path = APPPATH . 'views/birdchat_ai/birdchat_ai.php';        if (file_exists($birdchat_ai_file_path)) {            $this->load->view('birdchat_ai/birdchat_ai');        } else {            echo "File does not exist: " . $birdchat_ai_file_path;        }  }  public function analytics()  {       $analytics_file_path = APPPATH . 'views/analytics/analytics.php';        if (file_exists($analytics_file_path)) {            $this->load->view('analytics/analytics');        } else {            echo "File does not exist: " . $analytics_file_path;        }  }  public function contact()  {    $contact_file_path = APPPATH . 'views/contact/contact.php';        if (file_exists($contact_file_path)) {            $this->load->view('contact/contact');        } else {            echo "File does not exist: " . $contact_file_path;        }  }
}