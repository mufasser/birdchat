...
class Simplesupport extends Home
{
    ...
    public function reply($id=0)
    {
        ...
        $join = array('users' => 'fb_support_desk_reply.user_id=users.id,left','team_members' => 'fb_support_desk_reply.team_member_id=team_members.id,left');
        $table = "fb_support_desk_reply";
        $select = ['fb_support_desk_reply.*','brand_logo','users.name','team_members.name as team_member_name'];
