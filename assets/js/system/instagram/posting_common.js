...
$(function() {
    ...
    $(document).on('click','.select_media',function(e){
    	...
    	if($(this).hasClass('image'))
    	{
    		if(!$(this).hasClass('active')) {
    			if(image_list.length >= 10) return false;
    			$(this).addClass('active');
    			image_list.push(media_src);    
    		}  		
    		else {
    			$(this).removeClass('active');
    			image_list = image_list.filter(item => item !== media_src);     
    		}    		
    		$('#image_url').blur();
    	}
    	else
    	{
    		all_video_url.push(media_src);
    		if(!$(this).hasClass('active')){
	...
	$("#image_url_upload").uploadFile({
		...
		multiple:true,
		maxFileCount:10,
		acceptFiles:".jpg,.jpeg,.png,.mp4,.mov",
		onSuccess:function(files,data,xhr,pd)
		{
			var ext_array = data.split('.');
			var ext_type = ext_array.pop().trim().toLowerCase();
			var data_modified = base_url+"upload_caster/image_video/"+user_id+"/"+data;
			if(ext_type == 'mov' || ext_type == 'mp4')
			{
				var new_html = '<div class="col-4 col-md-3 col-lg-2 p-0 no-gutters"><video src="'+data_modified+'" width="100%" height="100" class="pr-1 pb-1 select_media image pointer border"></video></div>';
					      ...
					      $.ajax({
					       ...
					       success:function(response)
					       {
					         $(this).removeClass('btn-progress');
					       	 ...
					       	 if($(this).hasClass('image'))
					       	 {
					       	 	image_list = image_list.filter(item => item !== file_url);
					       	 	$("#image_url").blur();
					       	 }
									 else
									 {
    ...
    $(document).on('blur','#image_url',function(){
		var link='';
        if(image_list.length == 0) $("#image_url").val('');
        else{
        	link = image_list[image_list.length - 1].trim();    
      	...
      	if(image_list.length > 1) $("#image_edit_block").addClass("d_none");
      	if(link=='')  link = base_url+'assets/img/example-image.jpg';
        $(".only_preview_img").attr("src",link);

    });

    $(document).on('blur','#image_url_pasted',function(){			
        var pasted = $(this).val();   
        var exp = pasted.split(',');
        exp.forEach(function(src) {
        	src = src.trim();
        	var ext_array = src.split('.');
        	var ext_type = ext_array.pop().trim().toLowerCase();
            if(all_image_url.indexOf(src) === -1){
            	if(src != '')
            	{
		           if(ext_type == 'mov' || ext_type == 'mp4')
		           {
		           		var new_html = '<div class="col-4 col-md-3 col-lg-2 p-0 no-gutters"><video src="'+src+'" width="100%" height="100" class="pr-1 pb-1 select_media image pointer border"></video></div>';
		           		$("#image_block .upload_block").prepend(new_html);
		           		$('.select_media.image[src="'+src+'"]').click();
		           }
		           else
		           {
			           var new_html = '<div class="col-4 col-md-3 col-lg-2 p-0 no-gutters"><img src="'+src+'" width="100%" height="100" class="pr-1 pb-1 select_media image pointer"></div>';
			           $("#image_block .upload_block").prepend(new_html);
			           $('.select_media.image[src="'+src+'"]').click();
		           }
		    	   all_image_url.push(src); 
            	}
    		}
        });
    ...
    $(document).on('blur','#video_url',function(){
		...
		if(all_video_url.indexOf(link) === -1)
		{
    		var new_html = '<div class="col-4 col-md-3 col-lg-2 p-0 no-gutters"><video src="'+link+'" width="100%" height="100" class="pr-1 pb-1 select_media video pointer border active"></video></div>';
    		$("#video_block .upload_block").prepend(new_html);
    		all_video_url.push(link); 
		}
